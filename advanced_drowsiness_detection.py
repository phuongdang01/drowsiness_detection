...
while True:
    ...
    #============file (frame,mar)================
    os.makedirs("MAR_Result", exist_ok=True)
    with open("MAR_Result/mar_result.csv", "a", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        writer.writerow(["frame", "MAR"])
        writer.writerow([frame_count, mar])
    #============file (time,yawn)================
    os.makedirs("Time_yawn_Result", exist_ok=True)
    with open("Time_yawn_Result/time_yawn.csv", "a", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        writer.writerow(["time", "yawn_count"])
        writer.writerow([frame_count, yawn_counter])
    # ========== DROWSINESS ALERT ==========
    if is_drowsy:
